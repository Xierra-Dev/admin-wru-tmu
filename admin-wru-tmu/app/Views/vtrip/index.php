...
.filter-buttons {
    ...
    width: 100%;
    position: relative;
    z-index: 1000; /* Ensure filter buttons are above content */
}

.filter-buttons .dropdown {
    position: relative;
    z-index: 1001; /* Higher than filter-buttons */
}
...
.filter-btn {
    ...
    position: relative;
    z-index: 1002; /* Higher than dropdown container */
}
...
.filter-buttons .dropdown-menu {
    position: absolute;
    top: 100%;
    right: 0; /* Align dropdown to the right edge of button */
    left: auto; /* Remove left alignment */
    z-index: 1050 !important; /* Very high z-index for dropdown menus */
    display: none;
    float: left;
    min-width: 10rem;
    padding: 0.5rem 0;
    ...
    border-radius: 8px; /* Match M-Loc design */
    box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); /* Match M-Loc design */
}
...
.schedule-number {
    ...
    .schedule-content {
        width: 95%; /* Changed from 90% to 95% to fill the space */
        padding: 15px;
        display: flex;
        align-items: center;
        gap: 15px;
...
@media (max-width: 768px) {
    ...
    .filter-buttons {
        ...
        margin-bottom: 20px;
        flex-wrap: wrap;
        z-index: 1000 !important; /* Maintain high z-index on mobile */
    }
    
    .filter-btn {
        min-width: 70px;
        padding: 8px 16px;
        z-index: 1002 !important; /* Maintain high z-index on mobile */
    }
...
.modal.show {
    ...
    padding-top: calc(var(--global-navbar-height) + 10px) !important; /* Add top padding to avoid navbar overlap */
}
...
.searchable-dropdown-input {
    ...
    position: relative;
    z-index: 2147483641 !important; /* Override inherit */
}
...
.searchable-dropdown-menu {
    ...
    max-height: 250px;
    overflow-y: auto;
    z-index: 2147483647 !important; /* Maximum z-index - appears in front of everything */
    display: none;
...
.searchable-dropdown-arrow {
    ...
    z-index: 2147483642 !important; /* Override inherit */
}
...
.searchable-dropdown-display-input {
    ...
    cursor: pointer;
    transition: border-color 0.15s ease-in-out, box-shadow 0.15s ease-in-out;
    z-index: 2147483643 !important; /* Override inherit */
    position: relative;
}
...
.searchable-dropdown-search-input {
    ...
    background-color: #fff;
    transition: border-color 0.15s ease-in-out, box-shadow 0.15s ease-in-out;
    z-index: 2147483644 !important; /* Override inherit */
    position: relative;
}
...
.vtrip-main-container {
    z-index: 10 !important; /* Lower than filter buttons */
    position: relative;
}
...
.search-suggestion-item:last-child {
    ...
    transition: all 0.3s ease;
    border: none;
    background: rgba(92, 92, 92, 1); /* Gray color */
    border-radius: 0 25px 25px 0; /* Left straight, right rounded */
}
...
.btn-add {
    ...
    transition: all 0.3s ease;
    border: none;
    background: rgba(0, 157, 255, 1); /* Blue color */
    border-radius: 50px 8px 8px 50px; /* Right rounded, left straight */
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    class SearchableDropdown {
        ...
        selectItemByElement(element) {
            ...
            console.log('Selected item by element:', { value, text });
        }
